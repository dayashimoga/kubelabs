...
Coordinates single-container, multi-container pods/networks, K3s namespaces, and deterministic simulations.
Guarantees strict isolation and verified zero-residue cleanup.
"""
...
class EnvironmentBroker:
    ...
    def start_environment(self, sandbox_id: str, lab_spec: LabSpec) -> Dict[str, Any]:
        ...
        # Multi-container pod/network environment
        if env_spec.multi_container and self.is_podman_available:
            return self._start_multi_container(sandbox_id, lab_spec)

        # Single container environment
        if env_type in [EnvironmentType.CONTAINER, EnvironmentType.HYBRID] and self.is_podman_available:
            try:
                ...
                print(f"[EnvironmentBroker] Fallback to simulation due to: {exc}")
        ...
        env_record = {
            "sandbox_id": sandbox_id,
            "provider": "simulator",
            "classification": LabRuntimeClassification.SIMULATED if lab_spec.runtime_classification == LabRuntimeClassification.SIMULATED else LabRuntimeClassification.EMULATED,
            "simulator": sim,
            "created_at": time.time(),
            "lab_spec": lab_spec,
        }
    ...
    def _start_multi_container(self, sandbox_id: str, lab_spec: LabSpec) -> Dict[str, Any]:
        ...
        try:
            ...
            subprocess.run(
                [self.podman_executor.podman, "network", "create", f"--label=kubelabs.sandbox_id={sandbox_id}", network_name],
                check=False,
                capture_output=True,
                timeout=10,
            )
            ...
            for c_spec in multi_spec.containers:
                ...
                cmd = [
                    self.podman_executor.podman, "run", "-d",
                    "--name", c_name,
                ...
                if run_res.returncode == 0:
                    containers.append({"name": c_spec.name, "container_id": run_res.stdout.strip(), "container_name": c_name})
            ...
            print(f"[EnvironmentBroker] Multi-container launch failed: {exc}, fallback to simulation")
            sim = DeterministicSimulator(lab_spec.id, lab_spec.initial_state.seed_data)
            env_record = {
                "sandbox_id": sandbox_id,
                "provider": "simulator",
    ...
    def execute_command(self, sandbox_id: str, command: str, container_name: Optional[str] = None) -> Tuple[int, str, str]:
    ...
    def destroy_environment(self, sandbox_id: str) -> bool:
        """Destroy environment and clean up all associated containers, networks, and files."""
        ...
        if provider == "podman-single":
            ...
            if net_name and self.is_podman_available:
                try:
                    subprocess.run([self.podman_executor.podman, "network", "rm", "-f", net_name], capture_output=True, timeout=10)
                except Exception:
                    pass
    ...
    def verify_zero_residue(self, sandbox_id: str) -> Tuple[bool, List[str]]:
        ...
        try:
            # Check containers
            res = subprocess.run(
                [self.podman_executor.podman, "ps", "-a", "--filter", f"label=kubelabs.sandbox_id={sandbox_id}", "--format", "{{.Names}}"],
                capture_output=True,
                text=True,
                timeout=5,
            )
            found_containers = [line.strip() for line in res.stdout.strip().splitlines() if line.strip()]
            if found_containers:
                residue.extend([f"Container: {c}" for c in found_containers])

            # Check networks
            res_net = subprocess.run(
                [self.podman_executor.podman, "network", "ls", "--filter", f"label=kubelabs.sandbox_id={sandbox_id}", "--format", "{{.Name}}"],
                capture_output=True,
                text=True,
                timeout=5,
            )
            found_nets = [line.strip() for line in res_net.stdout.strip().splitlines() if line.strip()]
            if found_nets:
